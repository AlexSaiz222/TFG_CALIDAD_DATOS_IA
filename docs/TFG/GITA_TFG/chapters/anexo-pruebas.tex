...
\begin{itemize}
  ...
  \item Validar la lógica del dominio de análisis incremental (\texttt{AnalysisRun},
        \emph{baseline} y \emph{fingerprinting} de incidencias).
  \item Garantizar la integridad de los datos almacenados en PostgreSQL y
        MinIO, incluyendo el versionado de \emph{datasets}.
...
auditoría formal de accesibilidad \acs{WCAG} AA, pruebas de carga
...
\begin{description}
  ...
  \item[Integración.]  Comprueban la colaboración correcta entre Flask,
        SQLAlchemy, Celery, Redis y MinIO.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l >{\hsize=0.7\hsize}X >{\hsize=1.3\hsize}X l}
    \toprule
    ...
    BE-06 & \texttt{test\_metrics\_engine} +
      ...
      Comportamiento frente a casos límite de las siete métricas de
      calidad (completitud, unicidad, actualidad, exactitud sintáctica,
      equilibrio de clases, diversidad y consistencia lógica) y de la
      detección de valores atípicos como herramienta de diagnóstico
      (41 casos).
      & Implementada \\
    ...
    BE-09 & \texttt{test\_minio\_storage} &
      Subida, descarga e integridad de ficheros en MinIO.
      & Pendiente \\
    BE-10 & \texttt{test\_idor\_authorization} &
      ...
      \acs{JWT} (15 casos cubriendo \acs{IDOR} en lectura, escritura y
      enumeración).
      & Implementada \\
    \bottomrule
  \end{tabularx}
...
\subsection{Capa de dominio (análisis incremental)}

\begin{table}[H]
  \centering
  \small
  \caption{Suites de dominio de análisis incremental}
  ...
  \begin{tabularx}{\textwidth}{l >{\hsize=0.7\hsize}X >{\hsize=1.3\hsize}X l}
    ...
    DM-01 & \texttt{test\_fingerprinting} &
      ...
      ejecuciones (mismo \emph{issue} $\Rightarrow$ misma huella; 29 casos).
      & Implementada \\
...
\begin{itemize}
  ...
  \item \textbf{Docker compose.}  \emph{Stack} completo (PostgreSQL,
        Redis, MinIO, Celery) para pruebas de integración
        realista y manuales.
...
\begin{itemize}
  ...
  \item Ningún defecto de severidad \emph{bloqueante} o \emph{crítica}
        abierto.
  \item Cobertura de líneas $\geq 70$\,\% en el núcleo analítico
        (\texttt{services/metrics/}), donde se concentra la lógica de
        dominio de mayor riesgo.
  \item Suite \emph{smoke} verde en el último \emph{commit} de \texttt{main}.
\end{itemize}

\subsection*{Criterios de aceptación por \acs{HU}}
Cada \acs{HU} del \emph{backlog} (anexo~\ref{anx:backlog}) incluye
...
Las métricas e indicadores clave del plan de pruebas, recogidos en la
Tabla~\ref{tab:kpis-pruebas}, permiten evaluar cuantitativamente el proceso
de verificación: cobertura de código, tasa de paso, defectos críticos
abiertos y tiempo de ejecución de la batería.
%% =========================================================================
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l X l l}
    ...
    \textbf{KPI} & \textbf{Descripción} & \textbf{Objetivo} &
      \textbf{Estado} \\
    \midrule
    Cobertura motor de métricas & \texttt{pytest --cov} sobre \texttt{services/metrics/}.
                          & $\geq 70$\,\% & $\sim$80\,\% \\
    Cobertura global \emph{backend} & \texttt{pytest --cov} sobre \texttt{backend/}.
                          & --- & $\sim$36\,\% \\
    ...
    Densidad de defectos  & Defectos por cada mil líneas de código (KLOC) en el periodo.
                          & Decreciente   & N/A \\
    \bottomrule
  \end{tabularx}
\end{table}

El esfuerzo de pruebas se concentró en el núcleo analítico (motor de
métricas, cálculo del \emph{Quality Score}, \emph{fingerprinting} y
veredicto del \emph{Quality Gate}), que alcanza una cobertura de línea
en torno al 80\,\%.  La cobertura global del \emph{backend}, en torno al
36\,\%, es inferior porque los manejadores \acs{CRUD} de la \acs{API} y
las tareas Celery se validan sobre todo mediante pruebas de integración y
comprobaciones manuales; elevar su cobertura de línea queda recogido como
trabajo futuro.
...
mediante GitHub Actions, la generación automatizada de informes de
cobertura y el aumento de la cobertura de línea de los manejadores
\acs{CRUD} de la \acs{API} y las tareas Celery.
