...
\emph{frontend} que superó los cuarenta componentes reutilizables al
cierre del Sprint~4 y los cincuenta al cierre del proyecto.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l l X}
    ...
    TypeScript & 5.0 & Tipado estático que detecta errores en tiempo de
      ...
      por encima de los cincuenta componentes reutilizables. \\
...
\begin{definitionlist}
  ...
  \item[Asistente de \ac{IA}]  Claude Pro (Anthropic), utilizado como
    herramienta de apoyo durante el desarrollo en tres frentes
    delimitados: (i)~resolución de dudas técnicas puntuales sobre las
    bibliotecas empleadas (Flask, SQLAlchemy, Celery, Next.js, Chart.js)
    y depuración de errores concretos; (ii)~generación y revisión de
    fragmentos de código acotados (utilidades, validaciones, esqueletos
    de pruebas) que el autor ha verificado, ajustado e integrado
    manualmente al proyecto; y (iii)~revisión estilística y de
    coherencia interna de la presente memoria. Las decisiones de
    arquitectura, el diseño del catálogo de métricas, la selección y
    alineación con los estándares \ac{ISO}/IEC, la fórmula del
    \emph{Quality Score}, el sistema de \emph{fingerprinting} y la
    validación funcional del sistema son trabajo íntegro del autor.
\end{definitionlist}
...
\emph{story points} (SP) usando la escala de Fibonacci.  Las historias
de usuario se formalizaron al inicio del proyecto como marco de
planificación; las estimaciones en SP se ajustaron retrospectivamente
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l l X c c}
    ...
    \midrule
    \multicolumn{4}{r}{\textbf{Total SP planificados}} & \textbf{71} \\
    \multicolumn{4}{r}{\textbf{Total SP entregados (sin HU-15)}} & \textbf{68} \\
    \bottomrule
  \end{tabularx}
  \medskip\par\small
...
El backlog inicial planificado sumaba \textbf{71 puntos de historia},
de los cuales se entregaron \textbf{68}: HU-15 (panel de administración,
3 SP, prioridad baja) se descartó del alcance final ante la decisión de
...
mayor peso entre las entregadas son Evaluación (13~SP) y, en segundo
lugar, Proyectos, Datasets, Métricas, \emph{Fingerprinting} y \ac{EDA}
(8~SP cada una).
...
\begin{itemize}
  \item \textbf{\emph{Story Points} con escala de Fibonacci} para la
    ...
    base de 284~horas para las 71~SP del backlog.  Esta cifra no
...
\begin{table}[htbp]
  ...
  \textit{Nota.}~La columna SP de esta tabla agrega, por sprint, las
  ...
  usuario, de los cuales se entregaron 68~SP tras descartar HU-15 del
  alcance final; las tareas técnicas asociadas a infraestructura,
  arquitectura y \ac{API} \ac{REST} se computan adicionalmente como
  esfuerzo ejecutado, de modo que el total por sprint coincide
  numéricamente con los 71~SP del backlog inicial pero refleja una
  composición distinta (HU efectivamente entregadas más tareas
  técnicas).
\end{table}
...
Los sprints con mayor desviación fueron S5~(+38\,\%), S2~(+18\,\%) y
S1~(+15\,\%), causados respectivamente por la complejidad del sistema de
\emph{fingerprinting}, el parseo robusto de CSV y la configuración de la
...
el sprint más ajustado ($\approx$\,9\,\%), mientras que S4 se situó en torno
al $\approx$\,+16\,\%, próximo a la media global, dentro del margen de
