...
\begin{definitionlist}
  ...
  \item[Dinámica de trabajo]  Cada sprint se abría seleccionando historias del backlog
    y desglosándolas en tareas con estimación en Trello, y se cerraba con una
    reunión con los tutores, convocada bajo demanda según el ritmo real, para
    revisar el incremento y reajustar prioridades.  No hubo \emph{daily
    standup}: cada sesión empezaba con un repaso rápido de Trello.  La
    retrospectiva de cada sprint se recoge en el
...
\begin{itemize}
  \item \textbf{Editor de código:} Visual Studio Code, con extensiones para
    Python (Pylance), TypeScript, Docker y \LaTeX{} (LaTeX Workshop).

  \item \textbf{Entorno de ejecución local:} Docker Desktop sobre Windows~11,
    con Docker Compose para levantar los ocho servicios de forma simultánea,
    sin instalar dependencias en el sistema operativo anfitrión.

  \item \textbf{Gestor de paquetes \emph{frontend}:} pnpm, por su eficiencia
    en disco y velocidad de instalación frente a npm.

  \item \textbf{Gestor de paquetes \emph{backend}:} pip con fichero
    \texttt{requirements.txt} para las dependencias Python.

  \item \textbf{Pruebas:} pytest como \emph{framework} de pruebas del
    \emph{backend}, con soporte para pruebas unitarias y de integración.

  \item \textbf{Asistencia basada en \ac{IA}:} el asistente conversacional
    Claude se empleó como apoyo, sobre todo en la generación de código del
    \emph{frontend} y de las pruebas, además de en consultas técnicas y
    depuración.  Su uso se limitó a un rol de asistencia: la arquitectura, las
    decisiones técnicas y la revisión y validación de todo el código generado
...
La Figura~\ref{fig:gantt} muestra la distribución temporal de los sprints
a lo largo del curso; el resumen de hitos entregados en cada sprint se
recoge en la Figura~\ref{fig:roadmap} del Anexo~\ref{anx:backlog}.

\begin{sidewaysfigure}
  \centering
  \includegraphics[width=\textheight,height=\textwidth,keepaspectratio]{figures/gantt}
...
\begin{sidewaysfigure}
  \centering
  \includegraphics[width=\textheight,height=\textwidth,keepaspectratio]{figures/effort}
...
incidencia; esas incidencias se recopilaron más tarde en las
retrospectivas (Capítulo~\ref{chap:desarrollo}).

El análisis que se presenta a continuación tiene, por tanto, carácter
\textbf{retrospectivo}: a partir del historial de desarrollo del
repositorio y de las incidencias recogidas en las retrospectivas del
Capítulo~\ref{chap:desarrollo}, se reconstruyen los
...
contenedorización, pruebas automatizadas) junto con la recopilación
a posteriori de las incidencias en las retrospectivas.  El análisis detallado de
