...
\begin{description}
  \item[OE1 Módulo de ingesta y almacenamiento] (Cumplido)
    Se implementó soporte para \ac{CSV} con estrategia de
    \emph{fallback} múltiple (cuatro codificaciones combinadas con seis
    estrategias de separador y los dos motores de \emph{parsing} de
    Pandas), almacenamiento seguro en MinIO, extracción automática de
    esquema y versionado padre-hijo con indicador \texttt{is\_latest}.

  \item[OE2 Motor de métricas parametrizables] (Cumplido)
  ...
  \item[OE3 \acs{API} \acs{REST}ful con especificación formal] (Cumplido y superado)
  ...
  \item[OE4 Aplicación web con cuadros de mando interactivos]
    (Cumplido)  Se desarrollaron más de 20 páginas con cuadros de
  ...
  \item[OE5 Mecanismos de identificación de problemas]
    (Cumplido)  Se implementaron el sistema de \emph{issues} con
  ...
  \item[Objetivos transversales] (Cumplidos en su mayoría)
...
\textbf{Limitaciones encontradas:} el motor de métricas opera sobre
\emph{DataFrames} de Pandas cargados en memoria, lo que descarta procesar
\emph{datasets} de varios GB sin migrar a un motor distribuido; la
escalabilidad horizontal del \emph{worker} de Celery, aunque
arquitectónicamente permitida, no se ha validado en producción; la
autenticación se reduce a \ac{JWT} con usuario y contraseña, sin soporte
para \emph{Single Sign-On}, OAuth~2.0 ni autenticación multifactor; la
detección de valores atípicos se limita al método estadístico del \ac{IQR}
y no incorpora algoritmos como \emph{Isolation Forest} o DBSCAN; la
\emph{suite} de pruebas no incluye flujos \emph{end-to-end} automatizados
del \emph{frontend}, validados manualmente durante el desarrollo; y la
validación del sistema se ha realizado por el propio autor, sin
contraste empírico con usuarios externos del dominio que permita
evaluar la usabilidad de la interfaz o la idoneidad del catálogo de
métricas para casos de uso reales.
...
\ac{ISO}/IEC~25012~\citep{iso25012} e
\ac{ISO}/IEC~5259~\citep{iso5259} ha evitado definiciones arbitrarias y
...
salientes} se sitúa la exportación de informes a PDF y Excel, que
...
estrategia de \ac{QA} del proyecto.  En el plano analítico, la
...
de calidad de datos para \ac{IA}.  Queda trabajo por hacer
(notificaciones \emph{push} por \emph{WebSocket}, pruebas
\emph{end-to-end} del \emph{frontend}, validación con usuarios
externos), pero eso es precisamente lo que convierte un \ac{TFG} en el
punto de partida de algo mayor.
