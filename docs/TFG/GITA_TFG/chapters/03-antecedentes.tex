...
\drop{S}{aber} que los datos condicionan la calidad de un modelo de \ac{IA}~\citep{sambasivan2021, ng2021datacentric}
no basta si no se dispone de una forma sistemática de medirlo.  Durante las
...
\begin{itemize}
  ...
  \item \textbf{Apache Deequ}~\citep{schelter2018} es una biblioteca de
    ...
    no dispone de interfaz web y resulta desproporcionado para datasets de
    tamaño medio.
  ...
  \item \textbf{DQOps}~\citep{dqops} es una herramienta de código abierto
    ...
    monitorización de fuentes conectadas y no ha sido diseñada para proyectos de
    \ac{IA}.
  ...
  \item \textbf{Soda Core}~\citep{sodacore} es un \emph{framework} de código
    ...
    métricas específicas para \ac{IA}, de \emph{fingerprinting} de issues y de
    ocultación nativa de \ac{PII}.
\end{itemize}
...
\begingroup
% Junta los rótulos run-in de esta subsección para que la lista de
% herramientas no se desborde a páginas casi vacías.
\titlespacing*{\paragraph}{0pt}{0pt plus 0.3ex minus 0.2ex}{1em}
...
\ac{IA}, con licencia de pago y operación mayoritariamente \ac{SaaS}. En un segmento más maduro, las \emph{soluciones de
...
\dataqual en amplitud funcional, pero son productos propietarios de gobierno
del dato, no de evaluación de datasets de \ac{IA}/\ac{ML}.

\paragraph{Herramientas gestionadas en la nube.}
\ac{AWS} Glue Data Quality~\citep{awsgluedq}, construido sobre Apache Deequ,
integra reglas DQDL en el catálogo de \ac{AWS}~Glue sin gestión de
infraestructura, pero opera solo como \ac{SaaS} de pago por uso, exige que los
datos residan ya en \ac{AWS} y carece de métricas específicas para \ac{IA}.

\paragraph{Herramientas orientadas a \ac{ML}.}
\looseness=-1 % compone el párrafo una línea más corto para que no deje una línea huérfana
Evidently~AI~\citep{evidentlyai} monitoriza \emph{data drift} y equilibrio de
clases entre referencia y producción, no sobre un fichero estático; Pandera~\citep{pandera} valida esquemas de \emph{DataFrames}
de pandas, sin interfaz ni historial; e ydata-profiling~\citep{ydataprofiling}
genera informes HTML, sin motor de evaluación ni Quality Gates.
\endgroup
