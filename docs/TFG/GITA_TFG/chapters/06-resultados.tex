...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{X r}
    ...
    Ficheros de pruebas automatizadas               & 5     \\
    \bottomrule
  \end{tabularx}
\end{table}

Los cinco ficheros de prueba cubren las áreas de mayor riesgo del sistema:
\texttt{test\_auth.py} verifica el registro, el inicio de sesión y el refresco
de \emph{tokens} JWT; \texttt{test\_projects\_api.py} cubre el \ac{CRUD} de
proyectos; \texttt{test\_dataset\_versioning.py} y
\texttt{test\_dataset\_versioning\_api.py} comprueban la creación de versiones
y la trazabilidad padre-hijo; \texttt{test\_quality\_gate.py} valida la lógica
de umbrales y los tres estados del \emph{Quality Gate}.  La ejecución de las
pruebas se realiza mediante \texttt{pytest} desde el directorio
\texttt{backend/}, pudiendo generar informes de cobertura con la opción
\texttt{-{}-cov}.

Complementariamente, se realizaron pruebas manuales sobre los flujos
completos de la plataforma: registro de usuario, creación de proyecto, carga
de conjunto de datos, configuración de métricas, ejecución de evaluaciones y
consulta de resultados.  Estas pruebas verificaron la integración de todos
los componentes del sistema en un escenario realista, comprobando la
corrección de la interfaz de usuario en cada caso de uso principal.

El flujo completo de evaluación de extremo a extremo no está cubierto por
pruebas automatizadas y constituye una limitación reconocida
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l c c r r r}
    ...
    S5 — Quality Gate y cierre  & abr.\ 2026       & 10 & 40  & 55  & +37,5\,\% \\
    S6 — Documentación          & may.--jun.\ 2026 & — & 80  & 90  & +12,5\,\% \\
    \midrule
...
de seguridad, escalabilidad, mantenibilidad y portabilidad se cumplen; la
internacionalización completa del \emph{frontend} y las pruebas
\emph{end-to-end} quedan como trabajo futuro
...
\begin{enumerate}
  \item \textbf{Arquitectura extensible por \emph{plugins}.}  El patrón
    ...
    existente~\citep{design_patterns}.
  ...
  \item \textbf{\emph{Quality Gates} inspirados en SonarQube.}  Trasladan un
    concepto consolidado en la calidad del código~\citep{sonarqube} al
  ...
  \item \textbf{Solución \emph{full-stack} autoalojada.}  Todo el
    ...
    propietarios, manteniendo los datos bajo el control del usuario.
...
\begin{enumerate}
  ...
  \item \textbf{Sin detección de anomalías basada en \ac{ML}.}  Solo se
    emplea el método estadístico clásico \ac{IQR} para la detección de
    valores atípicos.  No se han implementado algoritmos como
    \emph{Isolation Forest} o DBSCAN.
