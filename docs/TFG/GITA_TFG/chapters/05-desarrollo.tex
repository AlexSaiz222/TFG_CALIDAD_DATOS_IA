...
\label{sec:sprint1}
Este primer sprint se desarrolló entre agosto y septiembre de 2025, a lo largo
de ocho semanas, con una carga estimada de 13 puntos de historia.
%% =========================================================================
...
\textbf{Cambio respecto a lo planificado}: la configuración de
...
La Figura~\ref{fig:arquitectura-componentes} presenta el diagrama de
componentes en notación \ac{UML}: separa los componentes de software
(\emph{frontend} y \emph{backend}) de la infraestructura (PostgreSQL, Redis y
MinIO), con sus interfaces provistas y requeridas y el protocolo de cada
comunicación.  El \emph{backend} es un único artefacto que se despliega como
servidor web (Gunicorn) y como procesos Celery; el detalle de los ocho
servicios se recoge en el Cuadro~\ref{tab:servicios}.

\enlargethispage{\baselineskip}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{img/arquitectura-componentes.png}
  \caption{Diagrama de componentes \acs{UML} de la plataforma \dataqual}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l c X}
    ...
    \texttt{redis}         & 6379 & \emph{Broker} de Celery y \emph{storage} del limitador de tasa \\
...
(archivos binarios) y Redis (\emph{broker} de Celery y limitador de tasa); cuando el usuario lanza
...
\label{sec:sprint2}
Este segundo sprint se desarrolló durante enero de 2026, a lo largo de cuatro
semanas, con una carga estimada de 18 puntos de historia.
%% =========================================================================
...
\textbf{Cambio respecto a lo planificado}: el parseo robusto de \ac{CSV}
...
La Figura~\ref{fig:diagrama-paquetes} muestra el diagrama de paquetes
\ac{UML} del \emph{backend}, con su organización en capas
(\texttt{api}~$\to$~\texttt{services}~$\to$~\texttt{metrics}/\texttt{models})
y las dependencias principales entre paquetes.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth,keepaspectratio]{img/diagrama_paquetes.png}
  \caption{Diagrama de paquetes \acs{UML} del \emph{backend}}
...
\begin{listing}[language=Python,style=apijson,float=!t,caption={Petición y respuesta de la API REST: creación de un proyecto},label={lst:api-ejemplo}]
POST /api/projects/
...
\label{sec:sprint3}
Este tercer sprint se desarrolló durante febrero de 2026, a lo largo de cuatro
semanas, con una carga estimada de 16 puntos de historia.
%% =========================================================================
...
\begin{sidewaysfigure}
  \centering
  \includegraphics[width=\textheight,height=\textwidth,keepaspectratio]{img/diagrama_clases.png}
...
\label{sec:sprint4}
Este cuarto sprint se desarrolló durante marzo de 2026, a lo largo de cuatro
semanas, con una carga estimada de 14 puntos de historia.
%% =========================================================================
...
\textbf{Cambios respecto a lo planificado}: la implementación de la
...
\label{sec:sprint5}
Este quinto sprint se desarrolló durante abril de 2026, a lo largo de cuatro
semanas, con una carga estimada de 10 puntos de historia.
%% =========================================================================
...
\textbf{Cambio respecto a lo planificado}: el diseño del
...
Con la incorporación del \emph{fingerprinting}, los \emph{Quality Gates} y la
séptima métrica, el Sprint~5 consolidó el núcleo diferenciador de \dataqual y
fue, a la vez, el de mayor complejidad de integración.  El
Cuadro~\ref{tab:sprint5-retro} sintetiza la valoración del equipo al cierre del
sprint.
...
\label{sec:sprint6}
Este sexto y último sprint se desarrolló entre mayo y junio de 2026, a lo largo
de ocho semanas, dedicadas a pruebas, refinamiento y documentación.
%% =========================================================================
...
El Cuadro~\ref{tab:sprint6-hu} recoge las actividades del sprint con su
estimación inicial y la dedicación real.
...
\textbf{Cambio respecto a lo planificado}: se detectaron tres
...
\begin{listing}[language=Python,float=!t,caption={Pruebas de determinismo y discriminación del \emph{fingerprinting}},label={lst:test-fingerprint}]
...
Las características no incluidas, detalladas con su motivo en el
Cuadro~\ref{tab:iso5259-cobertura}, responden a dos razones.  La mayoría no
...
que una herramienta externa no dispone (las dependientes del sistema, las
mixtas y varias de las adicionales).  Otras son técnicamente abordables, pero
quedaron fuera por priorización del proyecto y son candidatas para versiones
futuras.
...
\begin{enumerate}
  \item \textbf{Las características inherentes de \ac{ISO}/\ac{IEC}~25012}
    (completitud, exactitud, consistencia y actualidad), cuya correspondencia
    con las medidas de \ac{ISO}/\ac{IEC}~25024 y las métricas concretas que las
    implementan se detalla en el Cuadro~\ref{tab:iso25012-dataqual} del
    capítulo~\ref{chap:antecedentes}.  Proporcionan cobertura directa de los
...
\emph{end-to-end} queda como tarea pendiente y se reconoce como
limitación en el capítulo~\ref{chap:conclusiones}.  El plan de pruebas
