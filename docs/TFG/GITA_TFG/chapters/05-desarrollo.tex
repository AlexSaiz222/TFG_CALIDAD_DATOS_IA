...
El comportamiento esperado de \dataqual se especificó mediante historias de
usuario y casos de uso.  Las quince historias de usuario planificadas, con su
épica, prioridad y estimación en \emph{story points}, se recogen en la
Tabla~\ref{tab:backlog-hu} (capítulo~\ref{chap:metodologia},
sección~\ref{sec:backlog-inicial}), y el backlog completo con las tareas
técnicas en el Anexo~C; esta sección se centra en los casos de uso, que
describen las interacciones del usuario con el sistema.
...
%% =========================================================================
\section{Burndown global del proyecto}
...
tenían más puntos asignados que la media uniforme (S2:~18~\ac{SP}, S3:~16~\ac{SP},
frente a los 14{,}2~\ac{SP}/sprint de la distribución lineal), no a que se
completara menos trabajo del planificado.  Esta diferencia se mantiene en S4
y se anula en S5, donde ambas líneas convergen.  Sin embargo, registrar un avance superior al estimado en \ac{SP} no implica
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[pkg-tab, fill=green!20] at (api.north west) {API \ac{REST}};
...
representa en la Figura~\ref{fig:er-diagram} del
Anexo~\ref{anx:diagramas}.
...
de subida de dataset \ac{CSV} al sistema.
...
funcional y capacidad básica de ingesta de ficheros \ac{CSV}, constituyendo una
...
\emph{blueprints} representado en el diagrama de arquitectura
(Figura~\ref{fig:diagrama-arquitectura}).  La especificación completa
...
organización técnica \ac{REST}: \texttt{project\_metrics},
\texttt{project\_datasets} y \texttt{dashboard}), módulo de ingesta \ac{CSV},
...
\textbf{Qué fue mal:} el parseo robusto de \ac{CSV} fue mucho más complejo de lo
...
casos extremos (\ac{BOM}, delimitadores inusuales, caracteres especiales) para
...
pantallas principales y el módulo \ac{EDA}.  Incorporadas además tres métricas
...
\textbf{Qué fue bien:} el uso de Material \ac{UI} aceleró significativamente el
...
las funcionalidades principales: ingesta \ac{CSV}, siete métricas de
...
La batería se organiza en \textbf{doce módulos pytest} en
...
\emph{fingerprinting} con comparación contra línea base, y
autorización \ac{IDOR} contra accesos indebidos.  Las pruebas se
ejecutan con \texttt{pytest} desde \texttt{backend/} y pueden generar
informes de cobertura con la opción \texttt{-{}-cov}.  El plan
completo, con la pirámide de niveles, la cobertura por dominio y el
