...
\begin{table}[htbp]
  ...
  \small
  \begin{tabularx}{\textwidth}{>{\raggedright}p{3.8cm} >{\raggedright}p{3.8cm} X}
    \toprule
    ...
    \midrule
                    ...
                    & \texttt{diversity}            & Diversidad del conjunto de datos respecto a columnas clave \\
    \bottomrule
  \end{tabularx}
\end{table}
...
calidad de datos en contextos de analítica e \ac{IA}, con sus cinco partes
publicadas en 2024:
...
%% =========================================================================
\section[Alcance metodológico]{Alcance metodológico: evaluación algorítmica de la calidad inherente}
  ...
  \begin{tabularx}{\textwidth}{l X l X}
    ...
    Adicionales (MVP)
      & Equilibrio (\texttt{class\_balance}), Diversidad (\texttt{diversity}),
        Identificabilidad (enmascaramiento \ac{PII})
      & \checkmark~Implementadas
...
La \emph{similitud} presupone un conocimiento del espacio de características
del dominio que, desde una posición de análisis externo, no resulta accesible
sin la colaboración directa de la organización propietaria.
...
\begin{enumerate}
  ...
  \item \textbf{Diversidad del dataset} (\texttt{diversity}): evalúa, de
    ...
    los modelos entrenados con ese conjunto.

  \item \textbf{Identificabilidad de \ac{PII}}: el usuario designa las
    columnas sensibles al cargar el dataset, y la plataforma enmascara
    automáticamente sus valores en todos los resultados de evaluación.
    Su inclusión en el \ac{MVP} responde a la obligación legal establecida
    por el \ac{RGPD}~\citep{rgpd2016} y por el Reglamento Europeo de
    \ac{IA}~\citep{aiact2024} de proteger los datos personales utilizados
...
planos (como \ac{CSV}). Su configuración es compleja, no ha sido diseñado
específicamente para proyectos de \ac{IA}/\ac{ML} y carece de métricas como
el equilibrio de clases, la diversidad o la detección de outliers.
...
\ac{IA}/\ac{ML}, \emph{fingerprinting} de issues, versionado de datasets e
interfaz multiidioma (español e inglés), un conjunto de capacidades que
ninguna de las herramientas analizadas ofrece en su totalidad.  Great Expectations supera ampliamente a \dataqual en número de
...
\begin{enumerate}
  ...
  \item \textbf{Diseño centrado en IA/ML.} \dataqual incorpora métricas
    ...
    entropía de Shannon), diversidad del dataset (\texttt{diversity}) y
    evaluación de la actualidad temporal (\texttt{currentness}).
...
servidor y enmascaramiento de \ac{PII} conforme al \ac{RGPD}, todo ello
