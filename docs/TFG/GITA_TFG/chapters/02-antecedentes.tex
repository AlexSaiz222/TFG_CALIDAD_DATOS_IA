...
(microservicios orquestados con Docker Compose~\cite{docker}, patrón \ac{MVC}
en Flask~\cite{flask}, patrones de diseño clásicos~\cite{design_patterns} como
...
Celery~\cite{celery} y Redis~\cite{redis} como \emph{broker}, y una gestión
de datos que combina PostgreSQL~\cite{postgresql} con campos \texttt{JSONB}
para configuraciones flexibles, SQLAlchemy~\cite{sqlalchemy} como \ac{ORM} y
almacenamiento de objetos compatible con S3 mediante MinIO~\cite{minio} con
...
Material \ac{UI}~\cite{materialui} se comunica con el \emph{backend} mediante
