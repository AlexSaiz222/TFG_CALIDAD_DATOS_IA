...
definidas en \ac{ISO}/IEC~25012~\citep{iso25012} y en
\ac{ISO}/IEC~5259~\citep{iso5259}, complementadas con métricas específicas
para el contexto de \ac{IA} y \ac{ML}.  En concreto, el motor debe incluir
las siguientes \textbf{seis métricas evaluables} (que contribuyen al
\emph{Quality Score}):
...
valores atípicos} (\texttt{outliers}) basada en \ac{IQR}~\citep{tukey1977}
y Z-score.
Siguiendo la recomendación de \ac{ISO}/IEC~5259~\citep{iso5259} de no tratar
los \emph{outliers} como una dimensión de calidad per~se, esta clase
\textbf{no participará en el \emph{Quality Score}} sino que se utilizará
exclusivamente como herramienta de diagnóstico en el módulo de perfilado
de datos (\emph{EDA}).
...
en los listados.  Estos seis módulos funcionales se implementan como
\emph{blueprints} Flask principales; la convención REST de rutas anidadas
puede requerir \emph{blueprints} adicionales de organización técnica
(sub-rutas de proyecto y vistas de agregación) sin que ello amplíe el
...
\begin{enumerate}
  ...
  \item \textbf{\emph{Quality Gates}.}  Umbrales mínimos configurables que
    ...
    en \emph{pipelines} de \ac{MLOps}~\citep{kreuzberger2023} o de
    integración continua para bloquear el uso de conjuntos de datos de
...
El desarrollo está sujeto, además, a restricciones tecnológicas derivadas de
requisitos académicos y de las decisiones de arquitectura adoptadas:
...
\begin{itemize}
  ...
  \item El proyecto debe alinearse con los estándares
    ...
    \ac{ISO}/IEC~5259~\citep{iso5259} en lo relativo a las dimensiones
